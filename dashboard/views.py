...
def service_create(request):
    if request.method == 'POST':
        ...
        icon = request.FILES.get('icon')  # Changed from 'img' to 'icon'
...
def service_update(request, id):
    ...
    if request.method == 'POST':
        ...
        if 'icon' in request.FILES:  # Changed from 'img' to 'icon'
            service.img = request.FILES['icon']  # 'img' field in model, but 'icon' from form
        service.save()
